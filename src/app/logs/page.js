    ...
    useEffect(() => { 
        async function fetchLogs() {
            ...
            for (let i = 0; i < message.length; i++) {
                ...
                logs.push({
                    timeStamp: `${date.getDate()}-${date.getMonth() + 1}-${date.getFullYear()} ${((date) => {return `${date.getHours() % 12 || 12}:${date.getMinutes() < 10 ? '0' + date.getMinutes() : date.getMinutes()} ${date.getHours() >= 12 ? 'pm' : 'am'}`;
                    })(date)}`,
